...
export function ShoeTracker({ activities, shoes }: ShoeTrackerProps) {
    const shoeStats = useMemo(() => {
        ...
        // Filter shoes that were used in this period or are active
        // But the user specifically asked for shoes used in the filtered time period.
        return shoes
            .map(shoe => ({
                ...shoe,
            ...
            .filter(shoe => shoe.periodDistance > 0)
            .sort((a, b) => b.periodDistance - a.periodDistance);
    }, [activities, shoes]);

    if (shoeStats.length === 0) return null;
                ...
                {shoeStats.map(shoe => (
                    <div key={shoe.id} className="bg-black/40 rounded-2xl p-5 border border-white/5 hover:border-emerald-500/30 transition-all group">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h4 className="text-sm font-black text-white group-hover:text-emerald-400 transition-colors">{shoe.name}</h4>
                                {shoe.brand_name && (
